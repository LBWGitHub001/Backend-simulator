...
{
    busy_ = true;
    std::cout << "The train thread is Started!";
    while (!stop_)
    {
...
void MPTrainer::upload(std::vector<nn::DataSet>& data_sets)
{
    ...
    auto train_loader = torch::data::make_data_loader(std::move(train_ds),
                                                      64, /*!训练批量*/
                                                      true /*!打乱数据*/);
    auto val_set = std::vector(begin + train_size, data_sets.end());
    auto val_ds = MP::DataSet(val_set);
    auto val_loader = torch::data::make_data_loader(std::move(val_ds),
                                                    64, /*!训练批量*/
                                                    true /*!打乱数据*/);
    DataPair dp;
    dp.train_loader = std::move(val_loader);
    dp.val_loader = std::move(val_loader);
    dataset_queue_.push(dp);
